...
Type
     TFrameDrag = Class(TFrame)
          ...
          Procedure AvroIconMouseMove(Sender: TObject; Shift: TShiftState; X,
               Y: Integer);
...
Procedure TFrameDrag.AvroIconMouseDown(Sender: TObject;
...
Begin
     ...
     If pBackground <> '' Then Begin
          Background.Picture.LoadFromFile(pBackground);
          Background.Visible := True;
     End;

     If pAvroIcon <> '' Then Begin
...
Begin
     ...
     If AvroIcon.Visible Then Begin
          If (AvroIcon.Top < 0) Or (AvroIcon.Left < 0) Or (AvroIcon.Top + AvroIcon.Height > Background.Height) Or
               (AvroIcon.Left + AvroIcon.Width > Background.Width) Then AnyImageOutSide := True;
     End;

     If KM.Visible Then Begin
          If (KM.Top < 0) Or (KM.Left < 0) Or (KM.Top + KM.Height > Background.Height) Or
               (KM.Left + KM.Width > Background.Width) Then AnyImageOutSide := True;
     End;

     If KL.Visible Then Begin
          If (KL.Top < 0) Or (KL.Left < 0) Or (KL.Top + KL.Height > Background.Height) Or
               (KL.Left + KL.Width > Background.Width) Then AnyImageOutSide := True;
     End;

     If LayoutV.Visible Then Begin
          If (LayoutV.Top < 0) Or (LayoutV.Left < 0) Or (LayoutV.Top + LayoutV.Height > Background.Height) Or
               (LayoutV.Left + LayoutV.Width > Background.Width) Then AnyImageOutSide := True;
     End;

     If AvroMouse.Visible Then Begin
          If (AvroMouse.Top < 0) Or (AvroMouse.Left < 0) Or (AvroMouse.Top + AvroMouse.Height > Background.Height) Or
               (AvroMouse.Left + AvroMouse.Width > Background.Width) Then AnyImageOutSide := True;
     End;

     If Tools.Visible Then Begin
          If (Tools.Top < 0) Or (Tools.Left < 0) Or (Tools.Top + Tools.Height > Background.Height) Or
               (Tools.Left + Tools.Width > Background.Width) Then AnyImageOutSide := True;
     End;

     If Help.Visible Then Begin
          If (Help.Top < 0) Or (Help.Left < 0) Or (Help.Top + Help.Height > Background.Height) Or
               (Help.Left + Help.Width > Background.Width) Then AnyImageOutSide := True;
     End;

     If Exit.Visible Then Begin
          If (Exit.Top < 0) Or (Exit.Left < 0) Or (Exit.Top + Exit.Height > Background.Height) Or
               (Exit.Left + Exit.Width > Background.Width) Then AnyImageOutSide := True;
     End;


     ///////
     If AnyImageOutSide Then Begin
          If Application.MessageBox('Skin designer detected that at least one button is placed outside TopBar area.' + #10 + '' + #10 + 'Continue anyway?', 'Skin Designer', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2 + MB_APPLMODAL) = ID_YES Then
               Result := True;
     End
     Else
          Result := True;
