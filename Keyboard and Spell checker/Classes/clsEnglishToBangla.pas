...
Function TEnglishToBangla.Cnv(Const Compare: String;
...
Procedure TEnglishToBangla.CutText(Const inputEStr: String;
...
Begin
  ...
  Begin
    ...
    Case tStr Of
      ...
      '`':
        If i - 1 >= 1 Then
        Begin
          If (MidStr(inputEStr, i - 1, 1) = '.') Or
            (MidStr(inputEStr, i - 1, 1) = ':') Then
            p := i
          Else
            Break;
        End
  ...
  Begin
    ...
    Case tStr Of
      ...
      '`':
        If (MidStr(reverse_inputEStr, i + 1, 1) = ':') Or
          (MidStr(reverse_inputEStr, i + 1, 1) = '.') Then
          q := i
        Else
          Break;
      ':':
    ...
    Case tt Of
      ...
      'a', 'A':
        ...
        Else If (Begining = True) And (NextT <> '`') Then
          AddRs(b_AA)
        Else If (Consonent(PrevT) = False) And (PrevT <> 'a') And
          (NextT <> '`') Then
          AddRs(b_Y + b_AAkar)
        Else If NextT = '`' Then
          AddRsEx(b_AAkar, 2)
      ...
      'i':
        If ((Consonent(PrevT) = False) Or (Begining = True)) And
          (NextT <> '`') Then
          AddRs(b_I)
        Else If NextT = '`' Then
          AddRsEx(b_Ikar, 2)
      ...
      'Z':
        If PrevT = 'r' Then
        Begin
          If (Consonent(PrevTEx(2)) = True) And (PrevTEx(2) <> 'r') And
            (PrevTEx(2) <> 'y') And (PrevTEx(2) <> 'w') And
            (PrevTEx(2) <> 'x') Then
...
Begin
  ...
  If (Consonent(PrevT) = False) And (Vowel(NextTEx(1, 1)) = False) And
    (NextTEx(1, 1) <> 'r') And (NextTEx(1, 1) <> '') Then
  Begin
  ...
  If (Consonent(PrevT) = True) And (PrevT <> 'r') And (PrevT <> 'y') And
    (PrevT <> 'w') And (PrevT <> 'x') And (PrevT <> 'Z') Then
  Begin
