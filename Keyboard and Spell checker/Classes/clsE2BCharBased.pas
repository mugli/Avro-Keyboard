...
Begin
		 ...
		 Else Begin
					...
					If (Length(Middle_Main_T) <= Max_EnglishLength) And (Length(Middle_Main_T) > 0) Then Begin
							 ...
							 // frmPrevW.SelectFirstItem;
							 if CandidateDict.TryGetValue(Middle_Main_T, CandidateItem) And (SaveCandidate = 'YES') Then Begin
...
Begin
		 If (KeyCode = VK_SHIFT) Or (KeyCode = VK_RSHIFT) Or (KeyCode = VK_LSHIFT) Or (KeyCode = VK_LCONTROL) Or (KeyCode = VK_RCONTROL) Or (KeyCode = VK_CONTROL) Or (KeyCode = VK_MENU) Or
			 (KeyCode = VK_LMENU) Or (KeyCode = VK_RMENU) Or (IfWinKey = True) Then Begin
					Block := False;
					BlockLast := False;
					exit;
		 End;
...
Begin
		 Try
					Try
							 ...
							 for S in CandidateDict.Keys do begin
