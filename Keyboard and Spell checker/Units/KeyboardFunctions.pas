...
Begin
Result:=False;
     If (IfControl = True) And (RightAlterKeyDown = False) And (GeneralAlterKeyDown = False)
          And (LeftAlterKeyDown = False) Then
          Result := True
     Else
          Result := False;
End;
...
Result:=False;
     ...
     If keyState And 128 = 128 Then LWin := True;
     keyState := GetKeyState(VK_RWIN);
     If keyState And 128 = 128 Then RWin := True;
...
Result:=False;
     ...
     If keyState And 128 = 128 Then LControl := True;
     keyState := GetKeyState(VK_RCONTROL);
     If keyState And 128 = 128 Then RControl := True;
...
Begin
     ...
     If RightCtrlKeyDown = True Then SendInput_Up(VK_RCONTROL);
     If LeftCtrlKeyDown = True Then SendInput_Up(VK_LCONTROL);
...
Var
     i                        : integer;
Begin
     For I := 1 To Length(Keytext) Do Begin
...
Procedure RevertAltStates;
Begin
     If RightAlterKeyDown = True Then SendInput_Down(VK_RMENU);
     If LeftAlterKeyDown = True Then SendInput_Down(VK_LMENU);
     //   If GeneralAlterKeyDown = True Then SendInput_Down(VK_MENU);
End;

Procedure RevertCtrlStates;
Begin
     If RightCtrlKeyDown = True Then SendInput_Down(VK_RCONTROL);
