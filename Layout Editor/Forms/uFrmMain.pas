...
type
        TfrmMain = class(TForm)
                ...
                procedure Shape_QMouseDown(Sender: TObject;
        ...
        private
                ...
                procedure EncodeAndSaveImage(XML: IXMLDocument;
                  NodeName: string; FileName: string);
        public
                { Public declarations }
        end;
        ...
        for I := 0 to ComponentCount - 1 do
                ...
                begin
                        Child := KeyData.AddChild
                          ('Key_' + UpperCase((Components[I] as TShape).KeyName)
                          + '_Normal');
        ...
        try
                ...
                begin
                        Application.MessageBox
                          (PChar('Error occurred!' + #10 + #10 + E.Message),
                          PChar('Layout Editor'), MB_OK + MB_ICONHAND +
                ...
                try
                        ...
                        begin
                                Application.MessageBox
                                  ...
                                  MB_OK + MB_ICONHAND + MB_DEFBUTTON1 +
                                  MB_APPLMODAL);
                                exit;
                        end;
                        // ----------------------------------------------
                        ...
                        txtLayoutName.Text :=
                          VarToStr(XML.DocumentElement.ChildNodes.FindNode
                          ('LayoutName').ChildNodes[0].NodeValue);
                        txtDeveloper.Text :=
                          VarToStr(XML.DocumentElement.ChildNodes.FindNode
                          ('DeveloperName').ChildNodes[0].NodeValue);
                        txtVersion.Text :=
                          VarToStr(XML.DocumentElement.ChildNodes.FindNode
                          ('LayoutVersion').ChildNodes[0].NodeValue);
                        txtComment.Text :=
                          VarToStr(XML.DocumentElement.ChildNodes.FindNode
                          ('DeveloperComment').ChildNodes[0].NodeValue);

                        // Extract images
                        if XML.DocumentElement.ChildNodes.FindNode
                          ('ImageNormalShift') <> Nil then
                        begin
                                SStream :=
                                  TStringStream.Create
                                  (VarToStr(XML.DocumentElement.ChildNodes.
                                  FindNode('ImageNormalShift').NodeValue));
                                try
                                        DecodedData :=
                                        DecodeBase64(SStream.DataString);
                                        FStream :=
                                        TFileStream.Create
                                        (GetAvroDataDir +
                                        'tmpImage_Normal_Shift.bmp', fmCreate);
                                        try
                                        FStream.WriteBuffer(DecodedData[0],
                                        Length(DecodedData));
                        ...
                        if XML.DocumentElement.ChildNodes.FindNode
                          ('ImageAltGrShift') <> Nil then
                        begin
                                SStream :=
                                  TStringStream.Create
                                  (VarToStr(XML.DocumentElement.ChildNodes.
                                  FindNode('ImageAltGrShift').NodeValue));
                        ...
                        // Load Keys
                        Node := XML.DocumentElement.ChildNodes.FindNode
                          ('KeyData');

                        for I := 0 to Node.ChildNodes.Count - 1 do
                                if LowerCase
                                  (LeftStr(Node.ChildNodes[I].NodeName, 3)) <> 'num'
                                then
                                begin

                                        // Structure: Key_OEM1_Normal
                                        TrimLeftString :=
                                        MidStr(Node.ChildNodes[I].NodeName, 5,
                                        Length(Node.ChildNodes[I].NodeName));
                                        // OEM1_normal
                                        P := Pos('_', TrimLeftString);
                                        KeyName :=
                                        MidStr(TrimLeftString, 1, P - 1); // OEM
                                        Layer := LowerCase
                                        (MidStr(TrimLeftString, P + 1,
                                        Length(TrimLeftString)));
                                        // normal/Shift/AltGr/ShiftAltGr

                                        tmpShape :=
                                        FindComponent('Shape_' + KeyName)
                                        as TShape;

                                        if Node.ChildNodes[I]
                                        .ChildNodes.Count <= 0 then
                                        begin
                                        ...
                                        if Layer = 'normal' then
                                        tmpShape.Normal :=
                                        VarToStr(Node.ChildNodes[I].ChildNodes
                                        [0].NodeValue);
                                        if Layer = 'shift' then
                                        tmpShape.Shift :=
                                        VarToStr(Node.ChildNodes[I].ChildNodes
                        ...
                        begin
                                Application.MessageBox
                                  (PChar('Error occurred!' + #10 + #10 +
                                  E.Message), PChar('Layout Editor'),
                                  MB_OK + MB_ICONHAND + MB_DEFBUTTON1 +
                                  MB_APPLMODAL);
                                NewLayout;
                        end;
        ...
        for I := 0 to ComponentCount - 1 do
                if Components[I] is TShape then
                        if (Components[I] as TShape).KeyName = '' then
                                ...
                                begin
                                        KeyName :=
                                        UpperCase(MidStr(Name, 7,
                                        Length(Name)));
                                        CaptionNormal := LowerCase(KeyName);
                                        CaptionShift :=
                                        UpperCase(CaptionNormal);
                                end;
{$ENDREGION}
end;
...
begin
        ...
        begin
                retVal := Application.MessageBox
                  ('Save changes to the current keyboard layout?',
                  'Layout Editor', MB_YESNOCANCEL + +MB_ICONQUESTION +
                  MB_DEFBUTTON1 + MB_APPLMODAL);
...
begin
        ...
        begin
                Application.MessageBox
                  ('Please give a name to your layout first.', 'Layout Editor',
        ...
        begin
                Application.MessageBox
                  ...
                  'Layout Editor', MB_OK + MB_ICONHAND + MB_DEFBUTTON1 +
                  MB_APPLMODAL);
                exit;
        end;
...
begin
        ...
        for I := 0 to ComponentCount - 1 do
                if Components[I] is TShape then
                        if UpperCase((Components[I] as TShape).Name)
                          = UpperCase(ControlName) then
                        begin
                                (Components[I] as TShape).Selected := True;
                                fSelected := Components[I] as TShape;

                                if (Components[I] as TShape).CaptionShift <> '&'
                                then
                                        LabelShift.Caption :=
                                        (Components[I] as TShape).CaptionShift
                                else
                                        LabelShift.Caption := '&&';

                                LabelNormal.Caption := (Components[I] as TShape)
                                  .CaptionNormal;

                                txtNormal.Text :=
                                  (Components[I] as TShape).Normal;
                                txtShift.Text :=
                                  (Components[I] as TShape).Shift;
                                txtAltGr.Text :=
                                  (Components[I] as TShape).AltGr;
                                txtShiftAltGr.Text := (Components[I] as TShape)
                                  .ShiftAltGr;
